...
- A 模块标签栏当前只保留 `字段` 与用户自建栏目；`提示词`、`快捷字段` 在前端默认隐藏。
- Web 配置前端不再在启动时一次性加载全部模块数据，而是先加载壳层状态；用户点击哪个模块，再懒加载对应模块数据。
- Web 配置前端当前已挂接黑白风站点图标（`favicon.svg`），用于浏览器标签页与收藏夹入口的统一识别。
...
6. A 模块标签栏中不再显示 `提示词`、`快捷字段`。
7. 启动前端时不应一次拉取所有模块状态；切换模块时才触发对应模块加载。
8. 浏览器标签页可正常显示站点图标，不应再是默认空白页签图标。
