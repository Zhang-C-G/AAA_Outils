...
- 截图保存目录在页面中只读展示，默认不可修改。
- 页面提供“打开截图文件夹 / 打开目录”入口，直接跳到 assistant 截图目录。

## 透明度策略

- Web 端透明度改为 4 档点击切换：`0 / 50 / 75 / 100`
- 悬浮窗本体透明度也改为 4 档点击切换，不再使用拖动滑杆
- 存储与运行时会将透明度收敛到这 4 个固定档位
- 防录屏开启时，若 WDA 需要本地不透明窗口，透明度展示仍保留当前档位文案

## 悬浮窗行为

- 悬浮窗仅保留标题、状态栏、透明度档位按钮、`截图问答` 按钮与回答展示区。
...
5. 截图保存目录只读显示，且“打开目录”按钮可直接打开对应文件夹。
6. 透明度只能在 `0 / 50 / 75 / 100` 四档间切换。
7. 悬浮窗透明度也为点击式四档切换，不再是拖动滑杆。
8. 悬浮窗内不再显示“回答区”文字。
9. 回答长文本与代码段应自动换行，不出现整行横向拉长。
10. 录屏期间悬浮窗对本地持续可见，但截图 / 录屏结果中不应出现答案内容。
11. 触发外部截图快捷键时，不应在截图中留下问答悬浮窗黑块。
12. 按 `F1` 后悬浮窗应保持原位与同一实例更新，不允许回答阶段闪烁重建或坐标跳回固定位置。
