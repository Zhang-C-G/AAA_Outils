...
- 结构层面上，C 模块正式采用“列表 + 目录 + 正文工作区”模板，并默认继承该模板的交互与闭环规则；差异只允许出现在 C 自己的显示业务层。
...
- 本模块只允许读取 `notes_display/*.md` 作为展示数据源；写入动作仍归本模块自己的内容维护链路负责。
...
9. 用户手动调整过 F4 悬浮窗的位置和尺寸后，下次再次呼出应恢复到上一次的坐标与大小。
