...
- `2026-05-14`：继续优化 `docs/USER_DOC_SYSTEM_GUIDE.md`，新增文档简称表与“命令进入后的总判断树”，让用户先按简称分支判断，再决定阅读路径。
- `2026-05-14`：继续优化 `docs/USER_DOC_SYSTEM_GUIDE.md`，将命令树中的英文简称统一替换为中文简称，提升阅读利落度。
- `2026-05-14`：按“只保留总判断树与分支判断树”的目标重写 `docs/USER_DOC_SYSTEM_GUIDE.md`，移除额外说明与案例，只保留流程判断结构。
- `2026-05-14`：继续优化 `docs/USER_DOC_SYSTEM_GUIDE.md`，将原本分开的多棵判断树合并为一棵总树，避免用户在多处来回跳读。
- `2026-05-14`：修正文档树入口顺序：将 `总指挥` 固定到命令进入后的统一入口，移除后续分支里重复出现的 `总指挥` 节点，避免流程理解混乱。
- `2026-05-14`：继续修正 `docs/USER_DOC_SYSTEM_GUIDE.md`：将 `总指挥` 调整为“仅在进入实际任务后必须查看”的规则入口，明确它主要约束显性回答、状态显示与流程执行。
- `2026-05-14`：继续优化 `docs/USER_DOC_SYSTEM_GUIDE.md` 中“命中具体模块”分支，将“任务类型”和“第一阶段/第二阶段”改为前后判断关系，避免两者被误读成并列维度。
- `2026-05-14`：全面重构 `docs/USER_DOC_SYSTEM_GUIDE.md` 中“命中具体模块”分支：补入新配置项接入、结构层 bug 与运行层 bug 区分、第一阶段/第二阶段进入条件、偏好文档与风险知识库的触发条件。
- `2026-05-14`：继续细化 `docs/USER_DOC_SYSTEM_GUIDE.md` 中“老模块新增能力”分支，明确区分“补结构”与“补真实闭环”，避免阶段判断继续依赖口头理解。
- `2026-05-14`：继续修正 `docs/USER_DOC_SYSTEM_GUIDE.md` 中“bug 修复”分支，补入 incident 文档机制：先查 `docs/incidents/`，命中则先读；未命中但达到条件则先建 `TEMP_*.md`，修复确认后转 `RESOLVED_*.md` 并关闭临时文档。
- `2026-05-14`：继续压缩 `docs/USER_DOC_SYSTEM_GUIDE.md` 顶层任务分类：将“回归修复”并入“bug 修复”，将“新配置项接入”并入“老模块新增能力 -> 补真实闭环”，避免顶层分类重复。
- `2026-05-14`：按最新规则继续调整 `docs/USER_DOC_SYSTEM_GUIDE.md`：恢复“新配置项接入”作为独立任务类型，但明确它属于显型模块之外的配置型能力，仍需走第一阶段/第二阶段判断与四层同步。
- `2026-05-14`：修正 `docs/USER_DOC_SYSTEM_GUIDE.md` 中“新配置项接入”的定义：不再强调它是“显型模块之外”的特例，而是明确它虽然名字不叫模块，但构建逻辑仍按模块化、阶段化处理。
- `2026-05-20`：归档 B 笔记正文工作区近期迭代：新增正文头部双行工作带、小搜索框与手动保存入口，收紧 `最终版本 -> Markdown` 文本锚点定位，并同步模块文档、过程文档与相关私人偏好文档。
- `2026-05-14`：补强 `docs/USER_DOC_SYSTEM_GUIDE.md` 中“新模块”分支，写明第一阶段必须完成项、进入第二阶段条件、禁止进入条件，以及第二阶段的完成标准。
- `2026-05-14`：继续整理 `docs/USER_DOC_SYSTEM_GUIDE.md` 中“新模块”分支的写法：把第一阶段 / 第二阶段从长句改成并列关键点，保持树形阅读方式。
...
- `2026-05-15`：补齐语音自动分析的大模型流式输出：文本问答流式不再只限 `responses` 端点，现已兼容 `chat/completions` SSE，并在悬浮窗侧主动重绘流式回答。
- `2026-05-15`：加固 F4 笔记显示悬浮窗的位置与尺寸持久化：用户拖拽/缩放稳定后自动保存 `notes_overlay_x / y / w / h`，下次呼出继续复用上一次窗口位置与大小。
- `2026-05-15`：修复讯飞 `AppID` 回归性丢失：Assistant Web 保存链新增 `keep_xunfei_app_id` 与粘性文本保留逻辑，AHK 本地落盘也会优先保留磁盘中已有的 `xunfei_app_id`。
- `2026-05-15`：建立大模型流式输出监测机制：业务链路新增 `stream_metrics` 与 `assistant_stream_health_warning`，Testing 基线页新增 `stream_verdict / 首事件 / 首文本 / 更新次数 / 最大停顿` 展示。
- `2026-05-14`：修复 F 模块安装弹窗的扩展目录识别与打开动作：目录识别改为多路探测，`打开插件目录` 改为后端调用 `explorer.exe`，`打开扩展页` 改为后端触发 `chrome://extensions/`。
- `2026-05-18`：归档并模块化 B 笔记近期能力演进：补齐 `最终版本` 直接编辑、选区着色、列表项退回普通正文、自动保存兜底、当前笔记导出等稳定口径，同步新增独立第二阶段能力说明并回填相关私人偏好文档。
