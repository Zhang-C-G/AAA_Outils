...
- 简历自动填写模式：左侧分区导航，右侧字段表编辑；字段包含 `字段名/值/匹配别名/类型/说明`，并显示 JSON 预览。
...
- 按钮顺序：`快捷字段` / `笔记` / `截图发手机` / `截图问答` / `简历自动填写` / `快捷键` / `测试`
- `快捷字段`：进入主快捷字段页面（左侧栏目条目 + 右侧快捷键配置）
- `简历自动填写`：进入分区式简历 Profile 编辑页，供浏览器插件读取本地资料
